/* calc_fvocc.cpp : Measure contact occupancy over xx-ns time window. 
   Usage:
   g++ -Wall -std=c++11 calc_fvocc.cpp -o fvocc 
   Execute:
   ./fvocc name frate ui_dname resi_excl aocut iocut
   Variables: 
   - name is system. 
   - frate is frame rate. 
...
/* Declare functions */
void readForce(string ifname, vector<double> &force, vector<double> &time);
...
int main(int argc, char *argv[])
{
  ...
  vector<pair<double,double>> force_occ; // pair force and contact occupancy
 ...
 check: 
  ...
  for (k0=0;k0<2;k0++) { // two sets (in two files) of overlapping force data
    ...
    ifname="./in_data/"+name+"/f_"+name+"_"+to_string(frate)+"_"+to_string(k0)+"_interval.dat";
    force.clear(); time.clear();
    readForce(ifname,force,time);
    
    /* RELATE contacts and force: Average occupancy at intervals of frate. 
       Exclude residues/domains/types of contacts as specified. */
    //cout<<"time.back:"<<time.back()<<endl; 
    if ( (time.back()+frate/2) > ( ((int)occ[0].size())*dt) ) {
      force.pop_back(); time.pop_back();
    }

    if (nfrms!=(int)force.size()) {
      // entries in _0_ and _1_ did not match in size.
      // print warning last
      // force entry (which should have empty contact occ,
      // will not be output)
      //      cout<< "WARNING: force files had unequal # entries. This one was too long, so it won't be printed. Verify time entries match.  " <<endl;
    }
    assert((int)force.size()==(int)time.size()); 
    ...
    for (i=0;i<(int)occ.size();i++) { // loop through contacts
	  ...
	  ++ifrm;	  //cout<<" j:"<<j*dt<<" ifrm:"<<ifrm<<endl;
	}
  	//if (ifrm>nfrms) break; // ignore trail end of occ data
  	sum[ifrm]+=occ[i][j];
      } //     for (j=0;j<(int)occ[i].size();j++) {

      if (ifrm>(int)force.size()) //cout<< "error"<<endl; 
	ifrm=(int)force.size();
    ...
    // Pair force and occ data. Sort by force (pair *first).
    for (i=0;i<ifrm;i++) 
      tfo.push_back(make_tuple(time[i],force[i],sum_occ[i])); 
...
{ /* Read data in ifname, save to occ (bond,contact prescence=0 or 1).
  ...
  */
  ...
  cout<<" after average occ cutoff:"<<(int)occ.size()<<endl; 
...
/**********************************************************************/
void readForce(string ifname, vector<double> &force,vector<double> &time)
{ /* Read tows force data. Store and return. */

  double time0, fx, fy, fz, fspring;
  string line; 
  ifstream in(ifname.c_str());
  ...
  if (in.is_open()) {
	...
	if (line.find('#') != std::string::npos) continue;
	  ...
	  if (ss>>time0>>fx>>fy>>fz>>fspring) {
	    time.push_back(time0);
	    force.push_back(fspring);
	  }
	}
      } //     while (getline(in,line))
  } //   if (in.is_open()) {
  else cout<<"File not opened:"<<ifname<<endl; 
